              ...
              <div className="flex gap-5 max-md:flex-col max-md:gap-0 max-md:">
                <div className="flex flex-col w-[44%] max-md:ml-0 max-md:w-full  py-10">
                  <div className="flex gap-4  items-center p-3  border-slate-300">
                    <img src="https://source.unsplash.com/80x80?person" alt="person" className="rounded-full" />
                ...
                <div className="flex flex-col ml-5 md:ml-0 w-[56%] max-md:ml-0 max-md:w-full  py-6">
                  {!dataKos ? (
                    ...
                    <>
                      {dataKos.map((item) => (
                        <CardProduct id={item.id} kos_name={item.kos_name} rating={`${item.rating}`} address={item.address} kos_facilities={item.kos_facilities} photo_kos={item.photo_kos.main_kos_photo} hidden={true} price={item.price} />
                      ))}
                      z
                    </>
                  )}
